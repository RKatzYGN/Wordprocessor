...
export default function Editor({ content = '', editable = true, onChange }: EditorProps) {
  ...
  return <EditorInstance content={content} editable={editable} onChange={onChange} />;
}

function EditorInstance({ content = '', editable = true, onChange }: EditorProps) {
  ...
  return (
    <div
      className="border border-slate-200/80 print:border-none rounded-xl bg-white shadow-xs print:shadow-none transition-shadow overflow-hidden"
      onClick={() => editable && editor.chain().focus().run()}
    >
      {/* Floating Toolbar - Only visible when editable */}
      {editable && (
        <div
